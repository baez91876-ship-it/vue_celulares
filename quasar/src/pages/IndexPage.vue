<template>
  <q-page class="q-pa-md max-width-container">
    <section class="row q-col-gutter-md q-mb-lg">
        ...
        <q-card flat bordered class="bg-red-1 text-red-9 border-left-red">
          <q-card-section>
            <div class="text-h4 text-weight-bold">
              ${{ getStats().totalPendingBalance.toLocaleString() }}
            </div>
      ...
      <div class="col-12 col-sm-4">
        <q-card flat bordered class="bg-green-1 text-green-9 border-left-green">
          <q-card-section>
            <div class="text-h4 text-weight-bold">
              ${{ getStats().totalCollected.toLocaleString() }}
            </div>
    ...
    <section class="q-mb-xl">
      <div
        v-if="getFilteredServices().length === 0"
        class="text-center q-pa-xl text-grey-6 text-italic text-h6"
      >
      ...
      <div class="row q-col-gutter-md">
        ...
        >
          <q-card flat bordered class="service-card q-pa-sm" :class="getCardClass(service)">
            <div
            ...
            <q-card-section class="q-pt-xs">
              ...
              <div class="q-gutter-y-xs text-body2 text-grey-8">
                ...
                <div>
                  <strong>Precio Total:</strong> ${{ Number(service.price).toLocaleString() }}
                </div>

                <div
                  v-if="String(service.paymentStatus).toLowerCase() === 'abono'"
                  class="bg-amber-1 q-pa-sm rounded-borders q-mt-xs text-grey-9"
                >
                  <div v-if="Number(service.advancePayment1) > 0" class="text-caption">
                    • <strong>Abono 1:</strong> ${{
                      Number(service.advancePayment1).toLocaleString()
                    }}
                  </div>
                  <div v-if="Number(service.advancePayment2) > 0" class="text-caption">
                    • <strong>Abono 2:</strong> ${{
                      Number(service.advancePayment2).toLocaleString()
                    }}
                  </div>
                  ...
                  <div class="text-caption text-weight-bold text-amber-10">
                    Total Abonado: ${{
                      (
                        Number(service.advancePayment1 || 0) + Number(service.advancePayment2 || 0)
                      ).toLocaleString()
                    }}
                  </div>
                </div>
                ...
                <div
                  v-if="
                    String(service.paymentStatus).toLowerCase() === 'pendiente' ||
                    (String(service.paymentStatus).toLowerCase() === 'abono' &&
                      getPendingBalance(service) > 0)
                  "
                  class="q-mt-xs"
                >
                ...
                <div
                  v-else-if="
                    ...
                    getPendingBalance(service) === 0
                  "
                  class="q-mt-xs text-green text-weight-bold"
                >
                  ✓ Totalmente Cubierto
                </div>
              </div>
              ...
              <div
                ...
                class="bg-grey-2 q-pa-xs q-mt-sm rounded-borders text-caption text-grey-8 border-left-secondary"
              >
          ...
          <q-form @submit.prevent="saveService" class="q-gutter-y-md">
            ...
            <div v-if="String(form.paymentStatus).toLowerCase() === 'abono'" class="q-mt-sm">
              ...
              <div v-if="form.price" class="text-caption text-primary text-italic q-mt-xs">
                Total abonado: ${{
                  (
                    Number(form.advancePayment1 || 0) + Number(form.advancePayment2 || 0)
                  ).toLocaleString()
                }}
                | Saldo restante: ${{
                  Math.max(
            ...
            <div v-if="String(form.deviceStatus).toLowerCase() === 'entregado'" class="q-mt-sm">
              ...
              <q-rating v-model="form.rating" max="5" size="2.2em" color="amber" icon="star" />
            </div>
    ...
    <q-dialog v-model="showDeleteConfirm" persistent>
      <q-card style="width: 400px; max-width: 90vw">
        <q-card-section class="row items-center">
          ...
          <span class="q-ml-sm text-h6 text-weight-bold text-negative"
            >⚠️ Confirmar Eliminación</span
          >
        </q-card-section>
...
const validateForm = () => {
  ...
  if (payStatus === "abono") {
    ...
    if (totalAbonado > form.value.price) {
      errors.value.advancePayment2 =
        "La suma de ambos abonos no puede ser mayor al precio total de reparación";
      isValid = false;
    }
  }
